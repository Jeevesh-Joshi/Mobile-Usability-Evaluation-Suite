...
const btngrps = document.getElementById("controlbtns");
	...
	navigator.mediaDevices.getUserMedia(
		...
		.then((mediaStream) => {
			...
			mediaRecorder.onstop = () => {
				...
				recordedMedia = document.createElement(
				...
				downloadButton.classList.add('btn', 'btn-danger', 'text-center');
				...
				downloadButton.download = "Recording of username" + username;
				username += 1;
				...
				//cjanged here to remove the preview
				document.getElementById(
					`${selectedMedia}-recorder`).append(downloadButton);
				btngrps.appendChild(downloadButton);
			};
...
function stopRecording(thisButton, otherButton) {
	...
	thisButton.download = "Hello";
	thisButton.href = recordedMediaURL;
	URL.revokeObjectURL(recordedMedia);
	thisButton.disabled = true;
	otherButton.disabled = false;
